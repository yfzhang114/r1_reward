...
class NaiveReplayBuffer(ABC):
    ...
    def normalize(self, attribute: str, strategy) -> None:
        ...
        for i, item in enumerate(self):
            result = (items[i] - mean) * rstd + 1e-8  
            result = torch.where((result < lower_bound) | (result > upper_bound), torch.zeros_like(result), result)
            
            result = torch.where(torch.isnan(result) | torch.isinf(result) | (result > 10) | (result < -10), torch.zeros_like(result), result)
            
            setattr(item, attribute, result)
        return mean.item(), rstd.item()